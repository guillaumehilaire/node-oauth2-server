...
function usePasswordGrant (done) {
    // User credentials
    var params = {
        uname : this.req.body.username,
        pword : this.req.body.password,
        evt_id : this.req.body.eventId,
        fieldLogin : "email",
        fieldPassword: "password"
    };

    if (!params.uname || !params.pword || !params.evt_id) {
        return done(error('invalid_client',
            'Missing parameters. "username" and "password" and "eventId" are required'));
    }

    if( typeof this.req.goomeo.event  == "undefined" ) {
        return done(error('invalid_client',
            'Missing parameters. eventId is required'));
    }
    else {
        if( typeof this.req.goomeo.event.params != "undefined" && typeof this.req.goomeo.event.params.pe != "undefined" &&
            typeof this.req.goomeo.event.params.pe.fieldForLogin != "undefined" && typeof this.req.goomeo.event.params.pe.fieldForPassword != "undefined" ) {
            params.fieldLogin = this.req.goomeo.event.params.pe.fieldForLogin;
            params.fieldPassword = this.req.goomeo.event.params.pe.fieldForPassword;
        }
    }

    return this.model.getUser(params, function (err, user) {
        if (err) return done(error('server_error', false, err));
        if (!user) {
...
function useRefreshTokenGrant (done) {
    ...
    this.model.getRefreshToken(token, function (err, refreshToken) {
        ...
        if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
            ...
            return done(error('expired_grant', 'Refresh token has expired'));
        }
        ...
        var user_temp = refreshToken.userId.replace(/\'/g, "").replace(/ /g, "" ).replace(/\n/g, "").replace("{" , "" ).replace("}" , "" ).split(",");
        var user = {};


        user_temp.forEach(  function( item_user ){
            user[ item_user.split(":")[0] ] = item_user.split(":")[1];

        });

        return self.model.getUser(user.username, user.password, user.evt_id,  function (err, user) {
            if (err) return done(error('server_error', false, err));
            if (!user) {
                return done(error('invalid_grant', 'User credentials are invalid'));
            }
